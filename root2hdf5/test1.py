...
#        mceta_np = ak.to_numpy(mceta, allow_missing=True)
        ...
        with h5py.File(output_file, "w") as h5file:
            ...
            h5file.create_dataset("jpt", data=jpt)
            h5file.create_dataset("jphi", data=jphi)
            h5file.create_dataset("jeta", data=jeta)
            h5file.create_dataset("jmas", data=jmas)
            h5file.create_dataset("mcpt", data=mcpt)
            h5file.create_dataset("mcphi", data=mcphi)
            h5file.create_dataset("mceta", data=mceta)
